...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, TB2Common,
  StdCtrls, ExtCtrls, Menus, ActnList, ImgList, ComCtrls, {$IFDEF FPC}LMessages, TB2Delphi, {$ENDIF}
  TB2Item, TB2ExtItems, TB2Dock, TB2Toolbar, TB2MRU, math, tb2Control;
...
type
  ...
  TfrmDemo = class(TForm)
    ...
    EWordWrap: TTBItem;
    ESetFont: TTBItem;
    NBItem19: TTBSeparatorItem;
    NBItem20: TTBSeparatorItem;
    NBItem21: TTBSeparatorItem;
    NBItem27: TTBSeparatorItem;
    NBItem8: TTBSeparatorItem;
    NBItem9: TTBSeparatorItem;
    NBSeparatorItem8: TTBSeparatorItem;
    NBSeparatorItem9: TTBSeparatorItem;
  ...
  public
    ...
    procedure WndProc(var Message: {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF}); //override;
  end;
...
procedure TfrmDemo.OnHint(Sender: TObject);
begin
  StatusBar.Panels[0].Text := Application.Hint;
end;

procedure TfrmDemo.paint;
begin
 inherited paint;
end;

procedure TfrmDemo.WndProc(var Message: {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF});
var
  i : integer;
begin
  case Message.Msg of
...
procedure TfrmDemo.ItemClick(Sender: TObject);
begin
...
procedure TfrmDemo.MRUListClick(Sender: TObject; const Filename: String);
begin
  Application.MessageBox(PChar(Format('You selected "%s" from the MRU list.',
    [Filename])), 'MRUListClick', MB_OK or MB_ICONINFORMATION);
end;

procedure TfrmDemo.VMenuClick(Sender: TObject);
begin
  VStatusBar.Checked := StatusBar.Visible;
end;

procedure TfrmDemo.VStatusBarClick(Sender: TObject);
begin
...
procedure TfrmDemo.AlignClick(Sender: TObject);
begin
...
procedure TfrmDemo.BulletsItemClick(Sender: TObject);
begin
  BulletsItem.Checked := not BulletsItem.Checked;
end;

procedure TfrmDemo.BoldItemClick(Sender: TObject);
begin
  BoldItem.Checked := not BoldItem.Checked;
end;

procedure TfrmDemo.ItalicItemClick(Sender: TObject);
begin
  ItalicItem.Checked := not ItalicItem.Checked;
end;

procedure TfrmDemo.UnderlineItemClick(Sender: TObject);
begin
  UnderlineItem.Checked := not UnderlineItem.Checked;
end;

procedure TfrmDemo.PopupMenuBarButtonClick(Sender: TObject);
var
  P: TPoint;
begin
...
procedure TfrmDemo.PopupToolbarButtonClick(Sender: TObject);
var
  P: TPoint;
  a : TToolBar;
begin
