...
uses
  ...
  Windows, tb2Delphi, WinDelphi, LclIntf, LCLType, LCLStrConsts, Win32Int, InterfaceBase, LMessages,
  {$ENDIF}
  SysUtils, Classes;
...
begin
  ...
  {$ENDIF}
    ...
    if FBmpDC <> 0 then begin
      ...
      {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(FWndDC, 0, 0, FSize.cx, FSize.cy, FBmpDC, 0, 0, SRCCOPY);
      DeleteDC(FBmpDC);
      FBmpDC := 0;
    end;
  ...
  {$ENDIF}
    ...
    if NewStep > FCurStep then begin
      ...
      if FBlending then begin
        ...
        if not BOOL(SetWindowRgn(FWnd, Rgn, False)) then
          DeleteObject(Rgn);  { just in case }
        {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(FWndDC, X, Y, FSize.cx, FSize.cy, FBmpDC, 0, 0, SRCCOPY);
      end;
    end;
  end;
end;
