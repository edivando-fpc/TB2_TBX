...
uses
  SysUtils, Classes, Controls, Forms, ImgList,
  Menus, ActnList,
  Types,
  TB2Dock,
  TB2Item,
...
type
  ...
  public
    ...
    procedure GetTabOrderList(List: {$IFDEF FPC}TFPList{$ELSE}TList{$ENDIF}); override;
...
procedure HookProc(Code: THookProcCode; Wnd: HWND; WParam: WPARAM; LParam: LPARAM);
var
...
var
  ...
  begin
    ...
    {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(DC, R2.Left, R2.Top + Offset, R2.Right - R2.Left,
...
begin
  ...
  try
    TempBmp.Width  := R2.Width;
    TempBmp.Height := R2.Height;
...
begin
  if not AddMessage('CMControlListChange', CM_ControlListChange, Self, @Message) then begin {}inherited; exit; end;
  inherited;
  {$IFNDEF CLR}
...
begin
  if not AddMessage('CMShowHintChanged', CM_ShowHintChanged, Self, @Message) then begin {inherited;} exit; end;
  inherited;
  if ShowHint then
...
begin
  if not MouseCapture then
    FView.UpdateSelection({$IFDEF FPC}Types.{$ENDIF}Point(Low(Integer), Low(Integer)), True);
end;
...
begin
  if not(csDesigning in ComponentState) then begin
    P := ClientToScreen({$IFDEF FPC}Types.{$ENDIF}Point(X, Y));
...
begin
  if not AddMessage('WMMouseLeave', WM_MouseLeave, Self, @Message) then begin {inherited;} exit; end;
...
begin
  ...
  with TToolbarSizeData(FSizeData) do begin
    ...
    NCXDiff := ClientToScreen({$IFDEF FPC}Types.{$ENDIF}Point(0, 0)).X - Parent.Left;
...
procedure TTBCustomToolbar.GetTabOrderList(List: {$IFDEF FPC}TFPList{$ELSE}TList{$ENDIF});
var
  CtlList: {$IFDEF FPC}TFPList{$ELSE}TList{$ENDIF};
  I, J: Integer;
  CtlI, CtlJ: TWinControl;
begin
  inherited;
  { Remove off-edge items and their children from List }
  CtlList := {$IFDEF FPC}TFPList{$ELSE}TList{$ENDIF}.Create;
  try
...
begin
  if not AddMessage('CMWinIniChange', CM_WinIniChange, Self, @Message) then begin {inherited;} exit; end;
  inherited;
...
begin
  ...
  if (Message.Msg = CM_APPKEYDOWN) and Assigned(HookList) then begin
    ...
    if Assigned(ActiveForm) and (ActiveForm is TForm) and
      ...
      ActiveMDIChild := TForm(TForm(ActiveForm).ActiveMDIChild);
...
begin
  ...
  {$IFDEF FPC}
//  if HookCount = 0 then
//    tb2Delphi.TTB2ApplicationHook.ClassHookMainWindow(TTBCustomToolbar.MainWindowHook);
  {$ELSE}
//  if HookCount = 0 then
...
//  RemoveFromList(HookList, Self);
  ...
  {$IFDEF FPC}//@
//  if HookCount = 0 then
//    tb2Delphi.TB2ApplicationHook.UnhookMainWindow(TTBCustomToolbar.MainWindowHook);
  {$ELSE}
//  if HookCount = 0 then
