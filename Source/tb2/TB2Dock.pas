...
type
  ...
  private
    ...
    {$ENDIF}
    procedure WMNCCalcSize(var Message: TWMNCCalcSize); {$IFDEF NC_CALC}message WM_NCCALCSIZE;{$ENDIF}
    procedure WMNCPaint(var Message: TMessage); {$IFDEF NC_PAINT}message WM_NCPAINT;{$ENDIF}
  ...
  private
    ...
    procedure CMShowingChanged(var Message: TMessage); {$IFDEF SHOW_CHAN}message CM_SHOWINGCHANGED;{$ENDIF}
    procedure CMDialogKey(var Message: TCMDialogKey); //message CM_DIALOGKEY;
  ...
  private
    ...
    FSmoothDragging, LastVisibleChange : Boolean;
    ...
    procedure WMNCCalcSize(var Message: TWMNCCalcSize); {$IFDEF NC_CALC}message WM_NCCALCSIZE;{$ENDIF}
    procedure WMNCHitTest(var Message: TWMNCHitTest); {}message WM_NCHITTEST;
    procedure WMNCMouseLeave(var Message: TMessage); //message  WM_NCMOUSELEAVE; //$2A2;
...
begin
  ...
  {$ELSE}
  if {$IFDEF JR_D3} (Form is TCustomForm) and {$ENDIF}
     (TForm(Form).FormStyle = fsMDIChild) then
    for I := 0 to Screen.FormCount-1 do
      with Screen.Forms[I] do begin
        if FormStyle <> fsMDIForm then Continue;
          ...
          for I := 0 to FloatingToolWindows.Count-1 do begin
            with TTBCustomDockableWindow(FloatingToolWindows[I]) do
              ...
              if Assigned(Parent) and Parent.HandleAllocated and
                ...
                SetWindowOwner(Parent.Handle, {$IFDEF FPC}WidgetSet.AppHandle{$ELSE}Application.Handle{$ENDIF});
...
begin
  ...
  {$ENDIF}
    if (Wnd <> TaskActiveWindow) and (Wnd <> {$IFDEF FPC}WidgetSet.AppHandle{$ELSE}Application.Handle{$ENDIF}) and
...
begin
  if not AddMessage('WMMove', WM_MOVE, Self, @Message) then begin {}inherited; exit; end;
  inherited;
...
begin
  ...
  try
    ...
    {$IFDEF FPC}LclIntf.{$ELSE}Windows.{$ENDIF}GetClientRect(Handle, RC);
...
begin
  if not AddMessage('WMPrintClient', WM_PRINTCLIENT, Self, @Message) then begin {}inherited; exit; end;
  HandleWMPrintClient(PaintHandler, Message);
end;

procedure TTBDock.CMSysColorChange(var Message: TMessage);
begin
  if not AddMessage('CMSysColorChange', CM_SYSCOLORCHANGE, Self, @Message) then begin {}inherited; exit; end;
  inherited;
  if Assigned(FBackground) then
...
begin
  if not AddMessage('WMSysCommand', WM_SYSCOMMAND, Self, @Message) then begin {}inherited; exit; end;
...
{$ENDIF}
begin
  if not AddMessage('WMGetMinMaxInfo', WM_GetMinMaxInfo, Self, @Message) then begin {}inherited; exit; end;
  inherited;
...
begin
  if not AddMessage('CMShowingChanged', CM_ShowingChanged, Self, @Message) then begin {inherited;} exit; end;
  ...
  SetWindowPos(WindowHandle, 0, 0, 0, 0, 0, ShowFlags[isShowing and FShouldShow]);
end;

function TTBFloatingWindowParent.isShowing: Boolean;
begin
  {$IFDEF FPC}
  Result := (Self.Visible and (not (csDesigning in ComponentState)) or
...
var
  ...
  R, BR: TRect;
begin
  if not AddMessage('WMNCLButtonDown', WM_SYSCOMMAND, Self, @Message) then begin {}inherited; exit; end;
...
begin
  if not AddMessage('WMNCLButtonDblClk', WM_NCLButtonDblClk, Self, @Message) then begin {inherited;} exit; end;
...
begin
  if not AddMessage('WMNCRButtonUp', WM_NCRButtonUp, Self, @Message) then begin {inherited;} exit; end;
...
begin
  if not AddMessage('WMClose', WM_Close, Self, @Message) then begin {}inherited; exit; end;
...
begin
  if not AddMessage('WMActivate', WM_Activate, Self, @Message) then begin {}inherited; exit; end;
  if csDesigning in ComponentState then begin
    inherited;
    Exit;
  end;
...
begin
  if not AddMessage('WMMouseActivate', WM_MouseActivate, Self, @Message) then begin {}inherited; exit; end;
  if csDesigning in ComponentState then begin
    inherited;
    Exit;
  end;
...
procedure TTBCustomDockableWindow.WMMove(var Message: TWMMove);
  ...
  begin
    ...
    try
      ...
      {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(DC, 0, 0, CR.Right, CR.Bottom, BmpDC, 0, 0, SRCCOPY);
    finally
  ...
  if (Parent is TTBFloatingWindowParent) and Parent.HandleAllocated and
    ...
    if not ApplicationIsActive then
      Inactive := True
       ...
       (HWND(GetWindowLong(Parent.Handle, GWL_HWNDPARENT)) <> {$IFDEF FPC}WidgetSet.AppHandle{$ELSE}Application.Handle{$ENDIF}) then begin
...
var
  ...
  ParentForm: TTBCustomForm;
begin
  Result := isShowing and (FHidden = 0);
...
function TTBCustomDockableWindow.isShowing: Boolean;
begin
  {$IFDEF FPC}
  Result := (Self.Visible and (not (csDesigning in ComponentState)) or
    ((csDesigning in ComponentState)) and
...
begin
  ...
  if HandleAllocated then begin
    ...
    {$IFDEF FPC} if LastVisibleChange = Show then exit; {$ENDIF}
    ...
    {$IFDEF FPC} LastVisibleChange := Show; {$ENDIF}
        ...
        try
          ...
          try
            if SaveHandle <> 0 then begin
              ...
              {$IFDEF FPC}LclIntf.{$ELSE}Windows.{$ENDIF}SetParent(SaveHandle, AParent.Handle);
...
begin
  ...
  if Docked and CurrentDock.FAllowDrag and
    ...
    if I < DockedBorderSize + DragHandleSizes[CloseButtonWhenDocked, DragHandleStyle] then begin
      {$IFDEF FPC}LclIntf.{$ENDIF}SetCursor(LoadCursor(0, IDC_SIZEALL));
      Message.Result := 1;
      Exit;
    end;
  end;
...
var
  ...
  begin
    ...
    try
      ...
      {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(DC, R2.Left, R2.Top, R2.Right - R2.Left, R2.Bottom - R2.Top,
...
begin
  if not AddMessage('CMVisibleChanged', CM_VisibleChanged, Self, @Message) then begin {}inherited; exit; end;
...
var
      ...
      if WatchForSplit then begin
        if (MouseOverDock <> OriginalDock) or (NewDockRow <> OriginalDockRow) then
          ...
          if Abs(C) >= 10 then begin
            WatchForSplit := False;
            FDragSplitting := True;
            {$IFDEF FPC}LclIntf.{$ENDIF}SetCursor(LoadCursor(0, SplitCursors[SplitVertical]));
          end;
        end;
      end;
      FDockRow := NewDockRow;
  ...
  begin
    ...
    if PreventFloating and not Assigned(MouseOverDock) then begin
      ...
      {$IFDEF FPC}LclIntf.{$ENDIF}SetCursor(LoadCursor(0, IDC_NO));
    end
    else begin
      if FDragSplitting then
        {$IFDEF FPC}LclIntf.{$ENDIF}SetCursor(LoadCursor(0, SplitCursors[SplitVertical]))
      else
        {$IFDEF FPC}LclIntf.{$ENDIF}SetCursor(OldCursor);
    end;
...
begin
  if not AddMessage('CMMouseLeave', CM_MouseLeave, Self, @Message) then begin {}inherited; exit; end;
  inherited;
...
var
  ...
  PopupMenu: TPopupMenu;
begin
  if not AddMessage('WMContextMenu', WM_ContextMenu, Self, @Message) then begin {}inherited; exit; end;
...
begin
  if FFloating <> Value then begin
    if Value and not(csDesigning in ComponentState) then begin
      ...
      FFloatParent.Visible        := True;
      FFloatParent.Update;
      FFloatParent.FShouldShow    := True;
...
begin
  if FBitmapCache = nil then begin
    ...
    if not FTransparent then begin
      ...
      {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(FBitmapCache.Canvas.Handle, 0, 0, FBitmapCache.Width,
     ...
     {$IFDEF FPC}windelphi.{$ENDIF}SetBrushOrgEx(DC, DrawRect.Left - P.X, DrawRect.Top - P.Y, nil);
      ...
      while R2.Left < R2.Right do begin
        while R2.Top < R2.Bottom do begin
          {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(DC, R2.Left, R2.Top, UseBmp.Width, UseBmp.Height,
            DC2, 0, 0, SRCCOPY);
