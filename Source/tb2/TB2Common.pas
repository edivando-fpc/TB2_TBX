...
type
  ...
  TPaintHandlerProc = procedure(var Message: {$IFDEF FPC}TLMPaint{$ELSE}TWMPaint{$ENDIF}) of object;
...
begin
  ...
  try
    case MessageID of
//      WM_PAINT             : Result := True; //False;
//      WM_EraseBkgnd        : Result := True; //False;
      WM_Move              : exit;//Result := True; //False;
      WM_MouseMove         : exit;//Result := True; //False;
//      WM_NCCalcSize        : Result := True; //False;
      WM_NCHitTest         : exit;//Result := True; //False;
//      WM_NCMouseLeave      : Result := True; //False;
      CM_MouseLeave        : exit;//Result := True; //False;
//      CM_SHOWINGCHANGED    : Result := True; //False;
//      WM_NCLButtonDblClk   : Result := True; //False;
//      WM_NCLButtonDown     : Result := True; //False;
//      WM_NCPaint           : begin Result := True; exit; end;//False;
//      WM_NCRButtonUp       : Result := True; //False;
//      WM_Print             : Result := True; //False;
      WM_PrintClient       : Result := True; //False;
      WM_SetCursor         : exit; //Result := True; //False;
      CM_HintShow          : Result := True;
    else
    begin
      Result := False;
    end;
    ...
    Result := True;
  if Assigned(oMemo) then
  begin
    try
      oMemo.Lines.BeginUpdate;
      if Control is TWinControl then
        oMemo.Lines.Add(sMethod + ' ->>>>>>>>>>> Handle: ' + IntToStr(TWinControl(Control).Handle) + ' ->>>>>>> Name: ' + Control.ClassName)
      else
        oMemo.Lines.Add(sMethod + ' ->>>>>>>>>>> Component               ->>>>>>>>>>  Name: ' + Control.ClassName);
    finally
      oMemo.Lines.EndUpdate;
    end;
  end;
  except
  end;
...
var
  DC: HDC;
  Msg: {$IFDEF FPC}TLMPaint{$ELSE}TWMPaint{$ENDIF};
  SaveIndex: Integer;
begin
  {$IFNDEF CLR}
  DC := HDC(Message.WParam);
...
begin
  if (Win32MajorVersion >= 5) or
    ...
    {$ELSE}
  TBXUtils.GradientFillAvailable := True;
    {$ENDIF}
  end;
end;
...
begin
  ...
  {$ELSE}
   TBXUtils.GradientFill(DC, Vertexes, 2, GradientRect, 1, Mode);
  {$ENDIF}
end;
        ...
        if (GetClipBox(WndDC, {$IFDEF FPC}@{$ENDIF}ClipRect) <> Windows.ERROR) and
          ...
          if Bmp <> 0 then begin
            ...
            if BmpDC <> 0 then begin
              ...
              {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(WndDC, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top,
                BmpDC, 0, 0, SRCCOPY);
              Exit;
            end;
          end;
...
{$IFNDEF CLR}
begin
  Result := Char(CharLower({$IFDEF FPC}Char(Word(C)){$ELSE}Pointer(Word(C)){$ENDIF}));
end;
{$ELSE}
var
  AnsiBytes: TBytes;
