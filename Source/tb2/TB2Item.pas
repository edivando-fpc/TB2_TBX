...
type
  ...
  public
    ...
    procedure GetOffEdgeControlList(const List: {$IFDEF FPC}TFPList{$ELSE}TList{$ENDIF});
  ...
  private
    ...
    procedure WMGetObject(var Message: TMessage); {$IFDEF GETOBJ}message WM_GETOBJECT;{$ENDIF}
    ...
    procedure WMPaint(var Message: TWMPaint); {$IFDEF PAINT}message WM_PAINT;{$ENDIF}
    ...
    procedure WMTB2kAnimationEnded (var Message: TMessage); {$IFDEF TB2}message WM_TB2K_ANIMATIONENDED;{$ENDIF}
  protected
  ...
  public
    ...
    destructor Destroy; override;
    procedure GetChildren(Proc: TGetChildProc; Root: TComponent); override;
    function IsShortCut(var Message: TWMKey): Boolean; {$IFnDEF FPC}{override;}{$ENDIF}
...
begin
  if ClickWnd <> 0 then begin
    {$IFDEF FPC}LclIntf.{$ELSE}{$IFDEF ALLOCHWND_CLASSES}Classes.{$ENDIF}{$ENDIF} DeallocateHWnd(ClickWnd);
    ClickWnd := 0;
  end;
...
begin
  if ClickWnd = 0 then
    ClickWnd := {$IFDEF FPC}LclIntf.{$ELSE}{$IFDEF ALLOCHWND_CLASSES}Classes.{$ENDIF}{$ENDIF}AllocateHWnd(TTBCustomItem.ClickWndProc);
...
var
  ...
  var
    ...
    begin
      ...
      {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(DC, BR.Left, BR.Top, MenuCheckWidth, MenuCheckHeight,
...
//    Result := TBitmap(AllocPatternBitmap(clBtnFace, clBtnHighlight));
...
begin
  ...
  {$IFDEF FPC}LclIntf.{$ELSE}Windows.{$ENDIF}GetTextMetrics(Canvas.Handle, TextMetrics);
...
(*        Canvas.Pen.Color := clBtnText;
        Polyline(Canvas.Handle, {$IFDEF FPC}@{$ENDIF}Points[0], 7);
        Canvas.Pen.Color := clBtnHighlight;
        Polyline(Canvas.Handle, {$IFDEF FPC}@{$ENDIF}Points[7], 5);*)
      end;
  end;
end;
...
begin
  if (View = nil) or (Item = nil) then exit;
...
//        {$IFDEF FPC}windelphi.{$ENDIF}SetBrushOrgEx(DrawToDC, -WindowOrg.X, -WindowOrg.Y, nil);
      ...
      {$IFDEF FPC}
      MoveWindowOrg(DrawToDC, R1.Left, R1.Top);
      FWindow.Perform(LM_ERASEBKGND, WPARAM(DrawTo), 0);
      DrawCanvas := DrawTo;
      {$ELSE}
      Bmp := TBitmap.Create;
      Bmp.Width := R2.Right;
    ...
    if Offscreen then
      {$IFDEF FPC}LclIntf.{$ENDIF}BitBlt(DrawToDC, R1.Left, R1.Top, R2.Width, R2.Height, DrawCanvas.Handle,
        0, 0, SRCCOPY);
  finally
...
procedure TTBView.GetOffEdgeControlList(const List: {$IFDEF FPC}TFPList{$ELSE}TList{$ENDIF});
var
  I: Integer;
  Item: TTBCustomItem;
begin
...
begin
  ...
  if AExistingWnd <> 0 then
    ...
    FWnd := {$IFDEF FPC}LclIntf.{$ELSE}{$IFDEF ALLOCHWND_CLASSES}Classes.{$ENDIF}{$ENDIF} AllocateHWnd(WndProc);
    FCreatedWnd := True;
  end;
...
var
  ...
  begin
    { Don't continue if the mouse capture is lost, if a (modeless) top-level
      ...
      called. }
    Result := (GetCapture = FWnd) {$IFnDEF FPC}and (GetActiveWindow = OriginalActiveWindow){$ENDIF}
  ...
  begin
    ...
    {$IFDEF FPC}LclIntf.{$ENDIF}SetCursor(Cursor);
...
begin
  ...
  AppHandle := {$IFDEF FPC}GetDesktopWindow{$ELSE}Application.Handle{$ENDIF};
  if AppHandle <> 0 then
...
begin
  ...
  if HandleAllocated then
      {$IFDEF FPC}DestroyHandle;{$ELSE}DestroyWindowHandle;{$ENDIF}
  FreeAndNil(FView);
  inherited;
end;
...
begin
  ...
  if Assigned(FView) and (FView.ParentView = nil) and not FAccelsVisibilitySet then begin
...
begin
  if not AddMessage('WMGetObject', WM_GetObject, Self, @Message) then begin {inherited;} exit; end;
...
begin
  if not AddMessage('WMTB2kAnimationEnded', WM_TB2K_ANIMATIONENDED, Self, @Message) then begin {inherited;} exit; end;
...
begin
  if not AddMessage('WMPaint', WM_Paint, Self, @Message) then begin {inherited;} exit; end;
  { Must abort animation when a WM_PAINT message is received }
  TBEndAnimation(WindowHandle);
  {$IFDEF PAINT}inherited;{$ENDIF}
end;
