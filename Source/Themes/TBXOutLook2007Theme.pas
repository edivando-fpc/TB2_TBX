...
{**************************************************************************************
  ...
  The Office2003 Theme for TBX can be downloaded from

  http://www.rmklever.com/delphitbx.html
******************************************************************************}

unit TBXOutLook2007Theme;
...
//  TOffice2007Scheme = (osBlue, osSilver, osBlack, osUnknown);
  ...
  TTBXOutlook2007Theme = class(TTBXOffice2007ThemeBase)
  private
...
var
  ...
  Office2007Colors: array[osBlue..osBlack, cnDockColor..cnMDIAreaColor] of TColor = (
    ($FFDBBF, $FFEFE3, $FFD3B1, $D99D6F, $FFDBBF, $00FFC69A, $FFFFFF, $D99D6F,
    ...
    {cnBtnBodyColors_PopupParent_False} $007C7067 {.$FFEFE3},
    $7C7067, $A2E7FF, $AE9990)
    ); //320AFC
...
uses
  Controls, CommCtrl, Forms, SysUtils,
  Registry,
...
begin
  Result := MDIColors[GetOffice2007Scheme];
end;
...
{ TTBXOutlook2007Theme }

function TTBXOutlook2007Theme.GetBooleanMetrics(Index: Integer): Boolean;
begin
  case Index of
...
function TTBXOutlook2007Theme.GetIntegerMetrics(Index: Integer): Integer;
begin
  case Index of
...
function TTBXOutlook2007Theme.GetViewColor(AViewType: Integer): TColor;
begin
  Result := DockColor;
...
function TTBXOutlook2007Theme.GetBtnColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart; GradColor2: Boolean = False): TColor;
const
...
function TTBXOutlook2007Theme.GetPartColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart): TColor;
const
...
function TTBXOutlook2007Theme.GetItemColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipBody);
  if Result = clNone then Result := GetViewColor(ItemInfo.ViewType);
end;

function TTBXOutlook2007Theme.GetItemTextColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipText);
end;

function TTBXOutlook2007Theme.GetItemImageBackground(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetBtnColor(ItemInfo, ipBody);
  if Result = clNone then Result := GetViewColor(ItemInfo.ViewType);
end;

procedure TTBXOutlook2007Theme.GetViewBorder(ViewType: Integer; out Border: TPoint);
const
...
procedure TTBXOutlook2007Theme.GetMargins(MarginID: Integer; out Margins: TTBXMargins);
begin
  with Margins do
    case MarginID of
      MID_TOOLBARITEM:
...
procedure TTBXOutlook2007Theme.PaintBackgnd(Canvas: TCanvas; const ADockRect, ARect, AClipRect: TRect;
...
procedure TTBXOutlook2007Theme.PaintCaption(Canvas: TCanvas;
...
procedure TTBXOutlook2007Theme.PaintCheckMark(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  X, Y: Integer;
begin
...
procedure TTBXOutlook2007Theme.PaintChevron(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
const
...
procedure TTBXOutlook2007Theme.PaintEditButton(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXOutlook2007Theme.PaintEditFrame(Canvas: TCanvas;
...
procedure TTBXOutlook2007Theme.PaintDropDownArrow(Canvas: TCanvas;
...
procedure TTBXOutlook2007Theme.PaintButton(Canvas: TCanvas; const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  DC: HDC;
  R: TRect;
...
procedure TTBXOutlook2007Theme.PaintFloatingBorder(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXOutlook2007Theme.PaintFrame(Canvas: TCanvas; const ARect: TRect;
...
function TTBXOutlook2007Theme.GetImageOffset(Canvas: TCanvas;
...
procedure TTBXOutlook2007Theme.PaintImage(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXOutlook2007Theme.PaintMDIButton(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXOutlook2007Theme.PaintMenuItemFrame(Canvas: TCanvas;
  const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  R: TRect;
begin
...
//    PaintIrregularGradient(Handle, R, ToolbarColor1, ToolbarColor2, True);
    // DM
//    PaintIrregularGradient(Handle, R, EmbeddedDisabledColor, EmbeddedDisabledColor, True);
    PaintIrregularGradient(Handle, R, clBtnFace, clBtnFace, True);
    Inc(R.Left);
...
procedure TTBXOutlook2007Theme.PaintMenuItem(Canvas: TCanvas; const ARect: TRect; var ItemInfo: TTBXItemInfo);
var
  DC: HDC;
  R: TRect;
  X, Y: Integer;
...
procedure TTBXOutlook2007Theme.PaintPopupNCArea(Canvas: TCanvas; R: TRect; const PopupInfo: TTBXPopupInfo);
var
  PR: TRect;
begin
  with Canvas do
...
procedure TTBXOutlook2007Theme.PaintSeparator(Canvas: TCanvas; ARect: TRect;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // DM
        PaintIrregularGradient(DC, R, clBtnFace, clBtnFace, True);
//        PaintIrregularGradient(DC, R, EmbeddedDisabledColor, EmbeddedDisabledColor, True);
...
procedure TTBXOutlook2007Theme.PaintToolbarNCArea(Canvas: TCanvas; R: TRect; const ToolbarInfo: TTBXToolbarInfo);
const
...
procedure TTBXOutlook2007Theme.PaintDock(Canvas: TCanvas; const ClientRect,
  DockRect: TRect; DockPosition: Integer);
begin
  FillRectEx(Canvas.Handle, DockRect, DockColor);
end;

procedure TTBXOutlook2007Theme.PaintDockPanelNCArea(Canvas: TCanvas; R: TRect; const DockPanelInfo: TTBXDockPanelInfo);
...
procedure TTBXOutlook2007Theme.SetupColorCache;
const
...
begin
  ...
  begin
    ...
    begin
      DockColor := Office2007Colors[ColorScheme, cnDockColor];

      ToolbarColor1 := Office2007Colors[ColorScheme, cnToolbarColor1];
      ToolbarColor2 := Office2007Colors[ColorScheme, cnToolbarColor2];
      ToolbarFrameColor1 := Office2007Colors[ColorScheme, cnToolbarFrameColor1];
      ToolbarFrameColor2 := Office2007Colors[ColorScheme, cnToolbarFrameColor2];
      SeparatorColor1 := Office2007Colors[ColorScheme, cnSeparatorColor1];
      SeparatorColor2 := Office2007Colors[ColorScheme, cnSeparatorColor2];
      DragHandleColor1 := Office2007Colors[ColorScheme, cnDragHandleColor1];
      DragHandleColor2 := Office2007Colors[ColorScheme, cnDragHandleColor2];

      EmbeddedColor := Office2007Colors[ColorScheme, cnEmbeddedColor];
      EmbeddedFrameColor := Office2007Colors[ColorScheme, cnEmbeddedFrameColor];
      EmbeddedDisabledColor := Office2007Colors[ColorScheme, cnEmbeddedDisabledColor];

      PopupColor := Office2007Colors[ColorScheme, cnPopupColor];
      PopupFrameColor := Office2007Colors[ColorScheme, cnPopupFrameColor];

      DockPanelColor := ToolbarColor1;

      EnabledText := Office2007Colors[ColorScheme, cnEnabledText];
      DisabledText := Office2007Colors[ColorScheme, cnDisabledText];
      MenuItemFrame := Office2007Colors[ColorScheme, cnMenuItemFrame];

      WinFrameColors[wfpBorder] := Office2007Colors[ColorScheme, cnWinFrameColors_Border];
      ...
      BtnBodyColors[bisDisabled, False] := clNone;
      BtnBodyColors[bisDisabled, True] := clNone;
      BtnBodyColors[bisSelected, False] := Office2007Colors[ColorScheme, cnBtnBodyColors_Selected_False];
      BtnBodyColors[bisSelected, True] := Office2007Colors[ColorScheme, cnBtnBodyColors_Selected_True];
      BtnBodyColors[bisPressed, False] := Office2007Colors[ColorScheme, cnBtnBodyColors_Pressed_False];
      BtnBodyColors[bisPressed, True] := Office2007Colors[ColorScheme, cnBtnBodyColors_Pressed_True];
      BtnBodyColors[bisHot, False] := Office2007Colors[ColorScheme, cnBtnBodyColors_Hot_False];
      BtnBodyColors[bisHot, True] := Office2007Colors[ColorScheme, cnBtnBodyColors_Hot_True];
      ...
      BtnBodyColors[bisPopupParent, False] := Office2007Colors[ColorScheme, cnBtnBodyColors_PopupParent_False];
      BtnBodyColors[bisPopupParent, True] := Office2007Colors[ColorScheme, cnBtnBodyColors_PopupParent_True];
      ...
      MenuItemColors[misDisabled, ipFrame] := clNone;
      MenuItemColors[misHot, ipBody] := Office2007Colors[ColorScheme, cnMenuItemColors_Hot_Body];
      ...
      FMDIAreaColor := Office2007Colors[ColorScheme, cnMDIAreaColor];
    end
    else
    begin
      DockColor := clBtnFace;
    ...
    IrregularGradientValue := Office2007IrregularGradientValues[ColorScheme];
...
function TTBXOutlook2007Theme.GetPopupShadowType: Integer;
begin
  Result := PST_OFFICEXP;
end;

constructor TTBXOutlook2007Theme.Create(const AName: string);
begin
  inherited;
...
destructor TTBXOutlook2007Theme.Destroy;
begin
...
class function TTBXOutlook2007Theme.GetSubMenu: TObject;
begin
  Result := TTBXOffice2007Menu.Create(nil);
end;

procedure TTBXOutlook2007Theme.GetViewMargins(ViewType: Integer; out Margins: TTBXMargins);
begin
  Margins.LeftWidth := 0;
  Margins.TopHeight := 0;
...
procedure TTBXOutlook2007Theme.PaintPageScrollButton(Canvas: TCanvas;
...
procedure TTBXOutlook2007Theme.PaintFrameControl(Canvas: TCanvas; R: TRect; Kind, State: Integer; Params: Pointer);
var
  DC: HDC;
  X, Y: Integer;
  Pen, OldPen: HPen;
...
procedure TTBXOutlook2007Theme.PaintStatusBar(Canvas: TCanvas; R: TRect; Part: Integer);
...
procedure TTBXOutlook2007Theme.TBXSysCommand(var Message: {$IFDEF FPC}TLMessage{$ELSE}TMessage{$ENDIF} );
begin
  if Message.WParam = TSC_VIEWCHANGE then SetupColorCache;
end;

initialization
  RegisterTBXTheme('Outlook2007', TTBXOutlook2007Theme);
